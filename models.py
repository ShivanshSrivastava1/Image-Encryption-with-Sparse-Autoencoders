...
class downConvBlock_I(nn.Module):
    def __init__(self, in_size, out_size, scale_factor):
        ...
        #
        self.resConv = nn.Conv2d(self.in_size, self.out_size, 1, padding=0, stride=self.scale_factor) #kernel=1,padding=0
        self.conv1 =   nn.Conv2d(self.in_size, self.out_size, 3, padding=1, stride=1)
        self.conv2 =   nn.Conv2d(self.out_size, self.out_size, 5, padding=2, stride=self.scale_factor, groups=4) #kernel=5,padding=2
...
class downConvBlock_II(nn.Module):
    def __init__(self, in_size, out_size, scale_factor):
        ...
        self.conv2 =   nn.Conv2d(self.in_size, self.in_size, 5, padding=2, stride=1) #kernel=5,padding=2
        self.conv3 =   nn.Conv2d(self.in_size, self.out_size, 1, padding=0, stride=self.scale_factor) #kernel=1,padding=0
        self.bn    =   nn.BatchNorm2d(num_features=self.out_size)
    #
    def forward(self, x):
        res = self.resConv(x)
...
class upConvBlock_I(nn.Module):
    def __init__(self, in_size, out_size, scale_factor):
        ...
        self.resConv = nn.Conv2d(self.in_size, self.out_size, 1) #kernel=1
        self.conv1 = nn.Conv2d(self.in_size, self.out_size, 5, padding=2, stride=1) #kernel=5,padding=2
...
class upConvBlock_II(nn.Module):
    def __init__(self, in_size, out_size, scale_factor):
        ...
        self.up  = nn.Upsample(scale_factor=self.scale_factor, mode='bilinear', align_corners=False)
        self.resConv = nn.Conv2d(self.in_size, self.out_size, 1) #kernel=1
        self.conv1 = nn.Conv2d(self.in_size, self.in_size, 5, padding=2, stride=1)
        self.gn = nn.GroupNorm(num_groups=self.in_size, num_channels=self.in_size)  # Does it help?
        self.conv2 = nn.Conv2d(self.in_size, self.in_size, 3, padding=1, stride=1) #kernel=1,padding=0
        self.conv3 = nn.Conv2d(self.in_size, self.out_size, 1, padding=0, stride=1) # Howard and Shivansh
        # added this layer as part of their individual contribution
    ...
    def forward(self, x):
        ...
        x = self.conv3(x)
        x = x + res
        # As a result of the addition of conv3, the forward function was also updated accordingly
        #x = self.bn(x)
        return x
##################################
